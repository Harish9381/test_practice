...
def top_dir_with_max_movies(file_name):
    ...
    for movie in imdb_movie_data:
        ...
        if director in d:
            ...
            d[director] = 1
    return d
top_director = top_dir_with_max_movies('imdb_assignment/imdb_movies.json')
top_director = max(top_director.items(),key=lambda x: x[1])
...
def popular_genere_watched_by_most(file_name):
    ...
    for i in imdb_movie_data:
        ...
        if tuple(genere) in d:
            ...
            d[tuple(genere)] = 1
    return d            
top_genere_watched = popular_genere_watched_by_most('imdb_assignment/imdb_movies.json')
...
top_movies =  get_top_ten_movies_by_imdb_score('imdb_assignment/imdb_movies.json')
...
def least_watched_movie_by_imdb_score(file_name):
    ...
    return d
least_watched_movie_imdb_score = least_watched_movie_by_imdb_score('imdb_assignment/imdb_movies.json')
...
def get_best_director_in_first_hundred_movies(file_name):
    ...
    return d
best_director_in_hundred_movies = get_best_director_in_first_hundred_movies('imdb_assignment/imdb_movies.json')
